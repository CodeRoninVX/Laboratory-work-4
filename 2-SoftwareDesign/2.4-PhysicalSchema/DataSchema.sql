-- 2.4.1.1 Physical schema (PostgreSQL 16)
-- CREATE TABLE + PK + FK + CHECK (regex for text attributes)
...
-- ЗМІНА: TIME -> TEXT + regex (щоб виконати вимогу методички про regex на текст)
CREATE TABLE reminder_times (
    remindertime_id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    time_value TEXT NOT NULL,
    CONSTRAINT chk_reminder_times_value_regex
        CHECK (time_value ~ '^([01][0-9]|2[0-3]):[0-5][0-9]$')
);
